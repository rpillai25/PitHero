...
namespace PitHero
{
    ...
    public static class GameConfig
    {
        ...
        // Threat System (see PitHero/docs/ThreatSystem.md). All threat is in percent-of-max-HP units
        // so the numbers stay meaningful from pit 1 to pit 100+. Runtime-tunable for balance runs.
        /// <summary>Chance (0–1) that a monster attacks the current threat target instead of a random living ally.</summary>
        public static float ThreatTargetHitChance = 0.7f;
        /// <summary>Multiplier applied to every ally's threat at the end of each round (0.7 = −30%/round).</summary>
        public static float ThreatDecayPerRound = 0.7f;
        /// <summary>Threat per percent of an enemy's max HP dealt as damage (capped at 100% per hit).</summary>
        public static float ThreatPerDamagePercent = 0.5f;
        /// <summary>Threat per percent of an ally's max HP restored by a healing skill.</summary>
        public static float ThreatPerHealPercent = 1.0f;
        /// <summary>Threat gained per evasion, multiplied by the evader's evasion count this battle (15, 30, 45 …).</summary>
        public static float ThreatEvasionBase = 15f;
        /// <summary>Multiplier applied to all threat generated by a combatant whose job includes Knight.</summary>
        public static float ThreatKnightMultiplier = 2.0f;
        /// <summary>Default flat threat for an attack skill that declares no explicit ThreatValue (synergy skills).</summary>
        public static int ThreatSkillAttackDefault = 25;
        /// <summary>Default flat threat for a support/self skill that declares no explicit ThreatValue (synergy skills).</summary>
        public static int ThreatSkillSupportDefault = 30;
        /// <summary>Threat values below this snap to zero after decay so stale threat cannot linger forever.</summary>
        public static float ThreatFloor = 1f;
        /// <summary>
        /// Threat rescue: when the current threat target is a non-tank ally at or below this HP fraction,
        /// a Knight-flagged ally (pure or composite) prefers its highest-threat affordable attack skill
        /// over buffs/efficiency to pull the monsters back. Healing still takes precedence.
        /// </summary>
        public static float ThreatRescueHpPercent = 0.6f;
        /// <summary>
        /// Hold aggro: when true, a Knight-flagged ally that is not the current threat target always prefers its
        /// highest-threat affordable attack skill (tanks want the monsters' attention by default). When false only
        /// the wounded-target rescue tier fires.
        /// </summary>
        public static bool ThreatTankHoldAggro = true;
        /// <summary>
        /// Maintain margin for hold-aggro: a tank that already holds the target still re-fires its pull skill
        /// when its threat is below (highest rival threat × this factor), so it re-asserts the lead before a
        /// caster's steady output plus round decay overtakes it.
        /// </summary>
        public static float ThreatHoldMargin = 1.5f;
        ...
        /// <summary>Tint applied to the party HUD panel of the current battle threat target.</summary>
        public static readonly Color ThreatTargetHudTint = new Color(255, 105, 105);
        ...
        public const int NewGameStartingCornSeeds = 6; // Corn seeds in the seed inventory at new game start (issue #413: only unlocked crops are gifted)

        // New-game starter field (issue #413): a 3x3 block marked ReadyToTill with Wheat plans so the
        // starter Slime has work immediately. Inclusive tile bounds, below the starter buildings.
        public const int NewGameStarterFieldMinTileX = 121;
        public const int NewGameStarterFieldMaxTileX = 123;
        public const int NewGameStarterFieldMinTileY = 6;
        public const int NewGameStarterFieldMaxTileY = 8;
        ...
        public const int HighSpeedMaxStepsPerFrame = 16;         // Accumulator cap while running above 1x (live fast-forward or replay playback)

        // Speed ladder, shared by the live fast-forward button and the replay scrubber. Index 0 is
        // normal speed (fast-forward disengaged); the labels are the player-facing rendering of the
        // multipliers, so 2.5x reads as "2X".
        ...
        public const int SimulationVersion = 5; // v2: economy overhaul (issue #417) — chest gold rolls, dish gating, crop prices; v3: chest gold keyed on tier; v4: #420 farm water duty, meal HP/MP restore, closing serving-table drain, new-game defaults, replenish item targeting; v5: Food tab, fridge pre-stock and sell/purchase priority settings became commands
        ...
        public const int DebugStartingGold = 0;
