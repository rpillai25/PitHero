...
| `KitchenServiceLoopTests` | Headless end-to-end tavern service loop through the real `KitchenTaskCoordinator` + `TavernPatronComponent` (injected via `SetHeadlessServices`, no Nez scene): order → fetch → cook → plate → deliver → eat → pay → ticket retired; plus patience expiry before/after ordering (refund vs none), leave-after-cooking (payment stands), hire mid-dining, party-over-patron cook priority, mid-cook interrupt requeue |
