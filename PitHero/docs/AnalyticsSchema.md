...
| `crop_fridge_stocked` | `crop`, `qty`, `buildingId`, `source`, `monster`, `monsterType` | A runner moved `qty` units of a crop from a Crop Storage into the kitchen fridge (issue #386). One line **per crop per collect** — a batched pre-stock trip can emit up to 3. `source`: `"prestock"` (dedicated pre-stock trip) or `"ticket_topup"` (opportunistic top-up during an order's fetch trip). `buildingId` is the storage drawn from; `-1` when a routeless ticket top-up drew across every storage. Fires at the storage door (crops move at collect, walk back is cosmetic). Order-time fridge/storage reservations in `CreateTicket` are **not** logged — this event covers hand-carried movement only. Trip counts at carry level 1 ≈ line counts. |
