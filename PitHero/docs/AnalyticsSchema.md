...
| `monster_job_levelup` | `monster`, `monsterType`, `job`, `newLevel` | An allied monster's job skill level rose through completed tasks (issue #413: `current level × 10` tasks per level, cap 9). `job` is the `MonsterJob` enum name. One line per level gained. |
