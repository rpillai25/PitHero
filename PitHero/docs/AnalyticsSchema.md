...
| `crop_fridge_stocked` | `crop`, `qty`, `buildingId`, `source`, `monster`, `monsterType` | A runner unloaded `qty` units of a crop into the kitchen fridge (issue #386). One line **per crop per unload** — a batched pre-stock trip can emit up to 3. `source`: `"prestock"` (dedicated pre-stock trip; `buildingId` is the storage drawn from) or `"ticket_topup"` (opportunistic top-up during an order's fetch trip; `buildingId` is always `-1` — the cargo can aggregate several stops). Fires when the runner unloads at the fridge — the moment fridge stock actually rises — or on despawn-with-cargo (delivery teleports in). `qty` is the units that fit the fridge; overflow returned to storage is not logged. Order-time fridge/storage reservations in `CreateTicket` are **not** logged — this event covers hand-carried movement only. Trip counts at carry level 1 ≈ line counts. |
