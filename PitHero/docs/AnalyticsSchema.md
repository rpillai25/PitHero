...
- **Path:** `%LOCALAPPDATA%\PitHarvest\analytics\session_yyyyMMdd_HHmmss.jsonl`
...
| `chest_spawned` | `pitLevel`, `pitTier`, `x`, `y`, `chestLevel` (1–5), `item`, `kind`, `rarity`, optional `seedType` + `seedCount`, optional `stencilPatternId`, `gold` | Contents decided at spawn time. `item` is `null` for seed chests and stencil chests. `stencilPatternId` is present (non-null) only for stencil chests (issue #362); when present the chest is always level 4 (purple). `gold` (issue #417) is the pouch carried alongside the item — 0 for no pouch and always 0 for seed/stencil chests; it arrives via `gold_gained (source:"chest")` when the chest is opened. |
...
| `replenish` | `heroHP`, `heroMaxHP`, `heroMP`, `heroMaxMP`, `hpThreshold`, `mpThreshold`, `hpFlagged`, `mpFlagged`, `triggered` | One row per Replenish button press. `hpFlagged`/`mpFlagged` count party members (hero + mercs) below the respective threshold; `triggered:false` means nobody was flagged so no heal plan was requested (the exhausted-flags reset still happens). Any subsequent inn visit shows up as a separate `inn_sleep` with `cost > 0`. |
...
| `item_purchased` | `item`, `kind`, `rarity`, `qty`, `gold`, `source`, `gold_after` | Item bought back from the Second Chance vault. `qty` is the stack count for consumables, else 1. `gold` is the total spent (unit price × qty) and `gold_after` the funds left. `source`: `"auto_prepit"` (AutoItemPurchaseService, fired on the pit-entry jump). Also paired with `gold_spent (source:"item_purchase")`. |
| `gold_gained` | `amount`, `source`, `sessionTotal`, `currentGold` | `source`: `"battle"`, `"chest"` (issue #417 gold pouch), `"sell_item"`, `"sell_building"`, `"sell_crops"`, `"dish_sale"`, `"dish_tip"`, `"refund"`. `sessionTotal` resets each `session_start`. Every spend is logged as `gold_spent` (below), so `session_start.gold + Σ gold_gained − Σ gold_spent` reconciles to `currentGold`. `"refund"` includes a party meal canceled before cooking. `source:"sell_crops"` lines pair with per-stack `crop_sold` detail lines. |
| `gold_spent` | `amount`, `source`, `sessionTotal`, `currentGold` | One line per deduction via `GameStateService.SpendFunds`. `source`: `"seeds"` (manual + auto; pairs with `seed_purchased`), `"inn"` (pairs with `inn_sleep`), `"building"` (pairs with `building_created`), `"item_purchase"` (Second Chance vault, manual or auto pre-pit), `"crystal_purchase"`, `"crystal_forge"`, `"crystal_creation"`, `"merc_hire"`, `"monster"` (Monster House purchase), `"party_meal"` (hero pays at order time), `"artifact"` (Local artifacts only — Global artifacts are proof of wealth and deduct nothing). `sessionTotal` resets each `session_start`; `session_end` carries `goldSpentTotal`. Auto seed buys emit one `gold_spent` per seed but one aggregate `seed_purchased` per crop. |
| `vault_eviction` | `evicted`, `kind`, `rarity`, `qty`, `incoming` | Fired when the Second Chance vault (capacity 540 stacks) is full and a new item arrives. `evicted` is the item name that was removed; `qty` is its stack count at eviction. `incoming` is the name of the item that triggered the check — empty string when the incoming item itself was the weakest and was rejected instead (i.e., no eviction occurred, the arrival was silently discarded). Not fired on save-restore (`logEvictions:false`). |
...
| `threat` | `actor`, `actorType` (`"hero"`/`"merc"`), `source`, `amount`, `total` | One row per threat gain (see `ThreatSystem.md`). `source` is `"physical"`, a skill id, or `"evasion"`. `amount` is job-scaled (Knight ×2) and `total` is the actor's running threat after the gain, both truncated to integers in percent-of-max-HP units. Round decay and battle-end clears are not logged. |
| `provoke` | `actor`, `actorType`, `protected`, `reaction`, `mp`, `threatTotal` | One row per Knight Provoke cast (see `ThreatSystem.md`). `reaction: true` = the engine fired it out of turn because `protected` (a non-tank ally) dropped to ≤ `ThreatRescueHpPercent` after a monster hit; `false` = player-queued from the shortcut bar (`protected` is then `""`). `mp` is the MP actually spent; `threatTotal` is the caster's threat after the gain. The matching `threat` row has `source: "knight.provoke"`. Once per caster per battle. |
...
| `dish_unlocked` | `dish` | A dish became orderable through the dish progression (issue #417: recipe crops unlocked AND lifetime `dish_served` counts reaching the `DishUnlockConfig` requirements). Fires at the serving that crossed the last threshold (a crop unlock that completes a dish is announced at the next serving). `crop_sold` `gold` (issue #417) is the market-demand price actually paid, which can be below the base price. |

### Tavern dining (issue #319)
| `e` | Fields | Notes |
|---|---|---|
| `dish_served` | `dish`, `price`, `tip`, `isParty`, `deluxe` | One line per finished meal. `dish` is the `DishType` enum name. Patron meals (`isParty:false`) pair with `gold_gained (source:"dish_sale")` and, when `tip > 0`, `gold_gained (source:"dish_tip")`. Party meals (`isParty:true`) never tip; the hero pays at order time via `gold_spent (source:"party_meal")` (mercenary meals are free). A patron who leaves after cooking started (patience expiry or hired mid-dining) still pays via `gold_gained (source:"dish_sale")` but logs no `dish_served`. |
...
| `crop_fridge_stocked` | `crop`, `qty`, `buildingId`, `source`, `monster`, `monsterType` | A runner unloaded `qty` units of a crop into the kitchen fridge (issue #386). One line **per carry-queue entry per unload** — a batched pre-stock trip can emit up to 3, and a multi-stop ticket tour can emit one per (crop, storage) pair. `source`: `"prestock"` (dedicated pre-stock trip) or `"ticket_topup"` (opportunistic top-up during an order's fetch trip). `buildingId` is the storage the entry's hold was consumed from. Fires only at the fridge unload — the moment units physically leave storage and fridge stock rises; abandoned trips release their holds and log nothing. `qty` is the units that fit the fridge; overflow returned to storage is not logged. Order-time fridge/storage reservations in `CreateTicket` are **not** logged — this event covers hand-carried movement only. Trip counts at carry level 1 ≈ line counts. |
| `crop_fridge_returned` | `crop`, `qty` | The player sent a fridge stack back to crop storage via the Refrigerator window's Send to Crop Storage button (instant transfer, no runner). `qty` is the units that actually fit in storage. Selling from the fridge logs `crop_sold` instead. |
...
- **Not instrumented:** out-of-battle healing GOAP actions,
...
- **Economy:** `gold_gained` by `source` over `t` against `gold_spent` by `source`; the two reconcile exactly with `currentGold`.
