...
namespace PitHero.Tests
{
    ...
    public class ConsumableUsageTests
    {
        ...
        public void ConsumableUsage_APPotion_RestoresAP()
        {
            ...
            // Reduce AP to half
            var targetAP = maxAP / 2;
            while (hero.CurrentAP > targetAP)
            {
                hero.SpendAP(1);
            }
            
            var apAfterSpend = hero.CurrentAP;
            ...
            // Assert
            Assert.IsTrue(success, "Potion consumption should succeed");
            Assert.IsTrue(hero.CurrentAP > apAfterSpend, $"AP should increase from {apAfterSpend}, but is {hero.CurrentAP}");
        ...
        public void ConsumableUsage_MixPotion_RestoresBothHPAndAP()
        {
            ...
            Assert.IsTrue(hero.CurrentAP > apAfterSpend, $"AP should increase from {apAfterSpend}");
            Assert.IsTrue(hero.CurrentHP <= maxHP, $"HP should not exceed maxHP of {maxHP}");
            Assert.IsTrue(hero.CurrentAP <= maxAP, $"AP should not exceed maxAP of {maxAP}");
        }
