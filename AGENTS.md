...
PitHero is a horizontal RPG strip game built in **C# (.NET 8.0)** with **FNA + Nez** (not MonoGame). The game runs as a borderless window at the bottom of the screen at a virtual resolution of **1920×`GameConfig.VirtualHeight`** (currently **264**; was 360, then 296 — the OS window height follows the constant). A single hero adventures in a single growing pit while the player interacts with other desktop apps.
...
- Virtual resolution **1920×`GameConfig.VirtualHeight`** (currently **264**, flip the constant to compare heights; every tall UI window fits itself to `Stage.GetHeight()` at show time); game runs borderless, always-on-top, with optional click-through; maintain integer scaling for pixel-perfect rendering
...
- **UI views never compute or hold simulation truth.** Anything the sim reads (combat passives, protection rules, derived stats) is computed in the sim from sim state — never by a grid, tab, or window refresh. The 2026-09-14 divergence hunt found synergies (deflect, defense, stat bonuses) being detected by `InventoryGrid` only when a window refreshed; they now live in `HeroSynergyResolver`. If a UI class calls a hero/party mutator outside a command handler, that is the bug
- **A UI object that executes a command must resync from the sim first, and may be unconnected.** The UI overlay is built before the hero entity exists, so a grid whose window was never opened (every grid in a replay) is not bound to the hero at all; `InventoryGrid.SyncFromSimulation()` (called by the handlers' `GetGrid`) rebinds and refreshes before applying. Never let a handler act on a view's cached picture
- **Settings controls are commands too.** Radios, checkboxes, sliders and reorder lists that change a sim service (favorite dish, eat-at-tavern, fridge pre-stock size, sell/purchase priority) dispatch a command; a direct `svc.X = value` in `PitHero/UI` is the bug. A control synced from the service on open (`SetValueAndCommit`, `IsChecked`) guards its handler (`_refreshing`/`_syncingSlider`) so the sync is not recorded. Precedent (2026-09-17): the Food tab wrote `FavoriteDishId` directly, so a replay ordered the saved 35g dish instead of the 200g one picked live — `world` alone diverged by the price gap, only in the session where the tab was touched
- **A NewGame replay re-runs the new-game path, so the recording's start blob must carry everything that path reads.** The blob is captured at the top of `Begin`, *before* the hero entity exists, so any `GatherCurrentState` field sourced from the live hero is silently empty in it — and an empty field that some constructor quietly defaults is worse than a crash, because the replay builds a plausible wrong thing. Precedent (2026-09-17): `data.JobName` was only ever read off the hero entity, so a NewGame blob carried no job at all; `HeroDesign` defaults an empty job to `"Knight"`, and replaying a new game started as a Thief rebuilt the hero as a Knight — `hero` alone diverged **at tick 0** (recorded hp 75/mp 25 vs actual 95/22), RNG, party and world byte-identical. `GatherCurrentState` now also reads `design.JobName`. **The blob is captured before `LoadMap` and before every scene-scoped service is registered**, so this applies to services too, not just the hero: a gather that reads one must supply a null fallback *equal to the new-game default*, and a test should pin the two together (`FarmTaskCoordinatorTests.DefaultOrder_MatchesSaveDataDefault` is the pattern). When adding state the new-game path consumes, ask what it holds when no hero, scene or service exists yet
- **A handler never closes, pauses or unpauses from inside itself.** UI teardown that a command triggers (a dialog closing because the purchase filled the house) is deferred to the presentation pass (`AddMonsterDialog.Update`), and `PauseService` requests always queue while a session exists — a request raised mid-handler was once applied directly and never recorded, so the live game unpaused while the replay (dialog never open) stayed paused (2026-09-16)
...
- The **system save** (`Services/SystemSaveData.cs`, `system.bin`, owned by `ArtifactService`) holds player-level state that no hero or slot resets (**Global** artifacts). It has its own version and the same rules: append fields, read conditionally, keep unknown entries. **Local** artifacts (hero-specific, sim-read: crop growth, worker speed) live in `SaveData.LocalArtifacts` on the session `GameStateService` instead — see `PitHero/docs/ReplaySystem.md` "Artifacts". Replay files (`ReplayData`) are the one format allowed to drop old versions
- The **autosave** (`Services/AutoSaveService.cs`, `autosave_<HeroId as 8 hex digits>.bin`, same `SaveData` format) is written every `GameConfig.AutoSaveIntervalSeconds` from `MainGameScene.PresentationUpdate` (wall clock, never the sim step). There is **one autosave per hero**, keyed by `SaveData.HeroId` (stable across death and respawn), capped at `SaveLoadService.MaxAutoSaves` by evicting the least recently written. Gather the snapshot on the main thread (`GatherCurrentState`), write it on the worker through the **dedicated** autosave `FileDataStore` (the store caches one binary writer per instance), observe completion by polling — never call back from the worker, and delete files only on the main thread. `SaveLoadService.SaveAllowed` is the single save gate for manual and auto saves; anything that reads an autosave file first calls `AutoSaveService.WaitForCompletion()`. Full reference: `PitHero/docs/AutoSave.md`
...
- `PitHero/docs/AutoSave.md` — AutoSave (issue #409): wall-clock countdown in `PresentationUpdate`, gather-on-main / write-on-worker split with a dedicated `FileDataStore`, polling completion (no SynchronizationContext), the `SaveAllowed` gate table shared by manual and auto saves, the Load-mode-only AutoSave row, the SaveIcon indicator, why paused play is not change-gated, and the manual verification checklist
