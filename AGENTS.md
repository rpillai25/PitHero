...
- The **autosave** (`Services/AutoSaveService.cs`, `autosave.bin`, same `SaveData` format) is written every `GameConfig.AutoSaveIntervalSeconds` from `MainGameScene.PresentationUpdate` (wall clock, never the sim step). Gather the snapshot on the main thread (`GatherCurrentState`), write it on the worker through the **dedicated** autosave `FileDataStore` (the store caches one binary writer per instance), observe completion by polling — never call back from the worker. `SaveLoadService.SaveAllowed` is the single save gate for manual and auto saves; anything that reads the autosave file first calls `AutoSaveService.WaitForCompletion()`. Full reference: `PitHero/docs/AutoSave.md`
